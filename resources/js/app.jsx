...
const { sizes, designs, authenticated: initiallyAuthenticated = false } = window.shirtSettings;
...
function Admin() {
    ...
    const [isAdmin, setIsAdmin] = useState(initiallyAuthenticated);
    const query = new URLSearchParams({ size, design, page }).toString();
    useEffect(() => { let active = true; setLoading(true); setError(''); api(`/admin/submissions?${query}`).then(result => { if (active) { if (page > result.submissions.last_page) setPage(result.submissions.last_page); else { setData(result); setIsAdmin(!!result.authenticated); } } }).catch(e => { if (active) setError(e.message); }).finally(() => { if (active) setLoading(false); }); return () => { active = false; }; }, [query, refresh]);
    async function logout() { try { await api('/logout', {}); location.href = '/admin'; } catch (e) { setError(e.message); } }
    return <main className="admin"><div className="admin-heading"><div><p className="eyebrow">COLLECTION OVERVIEW</p><h1>Shirt selections<span>.</span></h1></div>{isAdmin ? <button className="secondary" onClick={logout}><LogOut size={16}/> Sign out</button> : <a className="secondary" href="/login">Admin sign in</a>}</div><div className="filters"><label>Shirt size<select value={size} onChange={e => { setSize(e.target.value); setPage(1); }}><option value="">All sizes</option>{sizes.map(s => <option key={s}>{s}</option>)}</select></label><label>Design<select value={design} onChange={e => { setDesign(e.target.value); setPage(1); }}><option value="">All designs</option>{designs.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</select></label><a className="primary" href={`/admin/export?${query}`}><Download size={17}/> Export CSV</a></div>{error && <p className="error" role="alert">{error} <button className="secondary" onClick={() => setRefresh(n => n + 1)}>Retry</button></p>}{notice && <p className="admin-notice" role="status">{notice}</p>}{!isAdmin && <p className="admin-notice" role="status">Anyone can view this list. Sign in as an admin to edit entries. Participants edit only their own selection from the collection form.</p>}{loading ? <p role="status">Loading selections…</p> : data && <><div className="stats"><div className="stat"><span>Submissions</span><strong>{data.total}</strong></div>{designs.map(d => <div className="stat" key={d.id}><span>{d.name} selections</span><strong>{data.designs[d.id] || 0}</strong></div>)}</div><div className="size-totals">{sizes.map(s => <div key={s}><span>{s}</span><strong>{data.sizes[s] || 0}</strong></div>)}</div><p className="hint">Totals reflect the current filters. A submission with both designs counts toward each design.</p><div className="table-wrap"><table><thead><tr><th>Full name</th><th>Display name</th><th>Display number</th><th>Size</th><th>Designs</th><th>Submitted</th>{isAdmin && <th>Actions</th>}</tr></thead><tbody>{data.submissions.data.map(row => <tr key={row.id}><td>{row.name}</td><td>{row.display_name ?? '?'}</td><td>{row.display_number ?? '?'}</td><td><span className="size-tag">{row.size}</span></td><td>{row.designs.map(id => `Design ${id}`).join(', ')}</td><td>{new Date(row.created_at).toLocaleString()}</td>{isAdmin && <td><button className="secondary" onClick={() => { setNotice(''); setEditing(row); }} aria-label={`Edit submission ${row.id} for ${row.name}`}>Edit</button></td>}</tr>)}</tbody></table>{!data.total && <div className="empty"><Shirt/><h2>No selections yet</h2><p>{size || design ? 'Try changing the filters.' : 'Submitted selections will appear here.'}</p></div>}</div><div className="pagination"><span>Page {data.submissions.current_page} of {data.submissions.last_page} · {data.total} submissions</span><button className="secondary" disabled={page <= 1} onClick={() => setPage(p => p - 1)}>Previous</button><button className="secondary" disabled={page >= data.submissions.last_page} onClick={() => setPage(p => p + 1)}>Next</button></div></>}{editing && isAdmin && <EditSubmission record={editing} onClose={() => setEditing(null)} onSaved={() => { setNotice(`Submission #${editing.id} updated.`); setEditing(null); setRefresh(n => n + 1); }}/>}</main>;
}
